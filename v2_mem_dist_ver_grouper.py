...
# tst
comm_latency = 0.001#45
average_tensor_size_if_not_provided = 1
comm_transfer_rate = 1#1000000 / (140 * 1024 * 1024 * 1024)
...
while improvement_achieved: #and iter < iterations_threshold:
    ...
    traversal_queue = []
    heapq.heappush(
        traversal_queue, (nodes_levels_scheduled_bottom[sink_node_name], sink_node_name))
    groups_times_till_now = [0] * no_of_desired_groups
    while traversal_queue:
        [current_node_start_time, current_node] = heapq.heappop(
            traversal_queue)
        ...
        for adj_node in rev_graph[current_node]:
            adj_node_group = nodes_groups[adj_node]
            nodes_levels_scheduled_bottom[adj_node] = max([current_node_start_time + (int(edges_weights[adj_node][current_node]) if current_node_group != nodes_groups[adj_node] else 0)
