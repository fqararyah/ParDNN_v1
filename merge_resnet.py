...
in3 = io_folder_path + 'src_snk_nodes_levels_low.txt'#'resnet_src_sink_nodes_levels_low.txt'
in4 = io_folder_path + 'blacklist_low.txt'
in5 = io_folder_path + 'inc_A_dot_src_sink.dot'#'resnet_src_sink_low.dot'
...
in4 = io_folder_path + 'blacklist_low.txt'
in5 = io_folder_path + 'nmt_src_sink_low.dot'
in6 = io_folder_path + 'colocation_32_low.txt' """
...
with open(in6) as f:
    for line in f:
        ...
        if splits[0] not in rev_collocated:
...
for collocation_src in collocated.keys():
    if collocation_src in graph:
        ...
        for coll_node in collocated[collocation_src]:
            h_zoltan_2_placement[coll_node] = '0'
...
with open(out1, 'w') as f:
    ...
    for node, part in h_zoltan_2_placement.items():
        if not node.startswith('^'):
            if node not in vanilla_placement:
                f.write(node + ' ' + part + '\n')
