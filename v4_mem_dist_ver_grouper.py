...
levels_work_sums = {}
nodes_weighted_levels = get_nodes_weighted_levels(
    graph=graph, grouped=True, edges_weights=edges_weights, nodes_groups=nodes_paths_mapping, is_rev=False, _nodes_in_degrees=nodes_in_degrees)
...
average_path_len = round(sum(paths_lengths) / len(paths_lengths))


print('average path len: ' + str(average_path_len))
...
# merging the groups
if to_be_merged_groups_weights:
...
for to_be_merged_group_i in range(0, len(to_be_merged_groups)):
    to_be_merged_group_index = to_be_merged_groups_indices[to_be_merged_group_i]
    to_be_merged_group = to_be_merged_groups[to_be_merged_group_index]
    if nodes_groups[to_be_merged_group[0]] != -1:
        continue
    ...
    for node in resident_nodes:
        if nodes_groups[node] == round_no and node not in var_nodes and node not in ref_nodes:
            heapq.heappush(resident_nodes_heap, ((
                nodes_comms[node][nodes_groups[node]] + 1) / (nodes_memory[node] + 0.1), node))
        ...
        if node_group == group_no:
            if not (node in sudo_nodes or node in no_op_nodes or node in ref_nodes or node in var_nodes):
                ...
                heapq.heappush(criteria_heap, ((
                    nodes_comms[node][node_group] + 1) / (candidate_node_mem_potential + 0.1), node))
                ...
                while overflow > 0 and (criteria_heap or big_nodes or resident_nodes_heap or collocated_nodes_heap):
                    ...
                    while ineffective_resident_nodes_heap:
                        if -ineffective_resident_nodes_heap[0][0] > scheduled_level:
                            ...
                            heapq.heappush(
                                resident_nodes_heap, (nodes_res_memory[res_node], res_node))
                    ...
                    if nodes_mem_potentials[node_name] > overflow and not heaps_are_empty:
                        heapq.heappush(
                            big_nodes, (nodes_comms[node_name][group_no], node_name))
                    else:
                        ...
                        if node_updated:
                            if from_big_nodes:
                                ...
                                heapq.heappush(criteria_heap, ((nodes_comms[node_name][node_group] + 1) / (
                                        ...
                                        for parent in rev_graph[node_name]:
                                            ...
                                            if candidate_node_level == parents_last_active_levels[parent][node_group]:
                                                ...
                                                if parent in ineffective_resident_nodes and parents_last_active_levels[parent][node_group] < candidate_node_level:
                                                    heapq.heappush(resident_nodes_heap, ((
                                                        nodes_comms[parent][node_group] + 1) / (nodes_memory[parent] + 0.1), parent))
                                                ...
                                                for child in graph[parent]:
                                                    ...
                                                    if nodes_groups[child] == node_group and child in visited_nodes and \
                                                        ...
                                                        heapq.heappush(criteria_heap, ((
                                                            nodes_comms[child][node_group] + 1) / (nodes_mem_potentials[child] + 0.1), child))
