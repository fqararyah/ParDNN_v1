...
in1 = io_folder_path + 'inc_A_dot_src_sink_low.dot'  # 'part_8_1799_src_sink.dot'
in2 = io_folder_path + 'timeline_step303_l.json'
# 'part_8_1799_src_sink_nodes_levels.txt'
in3 = io_folder_path + 'src_snk_nodes_levels_low.txt'
# 'rev_part_8_1799_src_sink_nodes_levels.txt'
in4 = io_folder_path + 'rev_src_snk_nodes_levels_low.txt'
...
in2 = io_folder_path + 'timeline-800.json'
# 'part_8_1799_src_sink_nodes_levels.txt'
    ...
    while(iterations < 2**BFS_max_depth and len(traversal_queue) > 0):
        ...
        if search_tree_level < search_tree_max_depth:
            ...
            if state_0_time < min_time:
                ...
                new_placement = True
                if any('tower_0/mixed_17x17x768d/branch_pool/avgpool/avgpool' in sublist for sublist in heavy_comm_groups):
                    print(min_time)
                    to_print = ''
                    for heavy_comm_group in heavy_comm_groups:
                        to_print = to_print + \
                            str(nodes_parts[heavy_comm_group[0]])
                    print(to_print)
...
while len(initial_groups) > no_of_desired_groups:
    ...
    if branch_main_path_indx == -1:
        for parent_node in analysis_graph[to_be_merged_group[0]].parents:
            if parent_node in tensors_sizes and int(tensors_sizes[parent_node]) > max_parent_or_child_tensor_size and nodes_groups[parent_node] != -1:
                max_parent_or_child_tensor_size = int(tensors_sizes[parent_node])
    ...
    if 'resnet_model/batch_normalization_26/assignmovingavg/mul' in to_be_merged_group:
        print(merge_destination_index)
        print(main_groups_indices)
        ...
        if adjustment_needed and len(to_be_merged_group) > 1 and not short_path_long_spreding:
            ...
            while traversal_queue:
                ...
                if current_node not in tmp_nodes_parts:
                    ...
                    for adj_node in graph[current_node]:
                        if int(analysis_graph[adj_node].level) < int(analysis_graph[branch_snk_node].level):
            ...
            if 'tower_0/mixed_17x17x768d/branch_pool/avgpool/avgpool' in to_be_merged_group:
                for heavy_comm_group in heavy_comm_groups:
                    print(main_branch_sum_in_targeted_levels)
                    print(an_alternative_max_time)
                    print(best_time)
...
print(nodes_groups['resnet_model/batch_normalization_26/assignmovingavg/mul'])
...
with open(out1, 'w') as f:
    for i in range(0, len(initial_groups)):
        ...
        for node in initial_groups[i]:
            ...
            if analysis_graph[node].level > 7:
                smm = smm + analysis_graph[node].duration
            if analysis_graph[node].level > 5 and analysis_graph[node].level < 500:
